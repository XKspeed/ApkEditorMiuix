...
fun TextEditorScreen(
    ...
    fun findNext() {
        ...
        val start = text.indexOf(findQuery, findPos + (if (findCount > 0) 0 else 0))
        ...
        topBar = {
            TopAppBar(
                ...
                actions = {
                    ...
                    Text(
                        if (dirty) "保存*" else "保存",
                        color = if (dirty) MiuixTheme.colorScheme.primary else MiuixTheme.colorScheme.primary,
    ...
    if (savedTip != null) {
        MiuixDialog(
            title = "保存",
            onDismiss = { savedTip = null },
        ) {
            Text(savedTip!!, color = MiuixTheme.colorScheme.onSurface)
            DialogActions(
                confirmText = "好的",
                onConfirm = { savedTip = null },
                cancelText = "",
                onCancel = { savedTip = null },
            )
        }
    }
}
